...
server.on('request', async (req, res) => {
    ...
    if (req.method === 'GET' && url.pathname === '/api/files') {
        ...
        try {
			...
			logMe('Serve fileList', `files = ${files}`);
            res.writeHead(200, { 'Content-Type': 'application/json' });
...
server.listen(port, hostname, () => {
    console.log(`ðŸŽ¬ Digital Prop Server running at http://localhost:${port}`);
    console.log(`ðŸ“œ View logs at http://localhost:${port}/api/logs`);
});
